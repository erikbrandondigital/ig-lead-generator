...
class InstaDM(object):
    ...
    def login(self, username, password):
        ...
        if self.__wait_for_element__(
            self.selectors["home_to_login_button"], "xpath", 10
        ):
            self.__get_element__(
        ...
        if not self.__wait_for_element__(self.selectors["username_field"], "name", 10):
            ...
            self.driver.find_element(
                By.NAME, self.selectors["password_field"]
            ).send_keys(password)
    ...
    def sendMessage(self, user, message, greeting=None):
        ...
        try:
            ...
            elements = self.driver.find_elements(
                By.XPATH, self.selectors["select_user"].format(user)
            )
    ...
    def __get_element__(self, element_tag, locator):
        ...
        try:
            ...
            if locator == "ID" and self.is_element_present(By.ID, element_tag):
                return WebDriverWait(dr, 15).until(
                    lambda d: dr.find_element(By.ID, element_tag)
                )
            elif locator == "NAME" and self.is_element_present(By.NAME, element_tag):
                return WebDriverWait(dr, 15).until(
                    lambda d: dr.find_element(By.NAME, element_tag)
                )
            elif locator == "XPATH" and self.is_element_present(By.XPATH, element_tag):
                return WebDriverWait(dr, 15).until(
                    lambda d: dr.find_element(By.XPATH, element_tag)
                )
            elif locator == "CSS" and self.is_element_present(
                By.CSS_SELECTOR, element_tag
            ):
                return WebDriverWait(dr, 15).until(
                    lambda d: dr.find_element(By.CSS_SELECTOR, element_tag)
                )
            else:
        ...
        for i in range(timeout):
            ...
            try:
                if locator == "ID" and self.is_element_present(By.ID, element_tag):
                    result = True
                    break
                elif locator == "NAME" and self.is_element_present(
                    By.NAME, element_tag
                ):
                    result = True
                    break
                elif locator == "XPATH" and self.is_element_present(
                    By.XPATH, element_tag
                ):
                    result = True
                    break
                elif locator == "CSS" and self.is_element_present(
                    By.CSS_SELECTORS, element_tag
                ):
                    result = True
                    break
                else:
